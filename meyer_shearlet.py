...
def _meyer_helper(x, realCoefficients=True, meyeraux_func=meyeraux):
    if realCoefficients:
        xa = np.abs(x)
    else:
...
def meyer_wavelet(x, realCoefficients=True, meyeraux_func=meyeraux):
    ...
    y : phihat
        values at given points x
    """
    y = np.sqrt(np.abs(_meyer_helper(x, realCoefficients, meyeraux_func))**2 +
                np.abs(_meyer_helper(2*x, realCoefficients, meyeraux_func))**2)
    return y


def meyer_shearlet_spect(x, y, a, s, realCoefficients=True,
    ...
    # compute spectrum
    Psi = meyer_wavelet(x, realCoefficients, meyeraux_func) * \
        bump(y/xx, meyeraux_func)
    return Psi


def meyer_smooth_shearlet_spect(x, y, a, s, realCoefficients=True,
                             meyeraux_func=meyeraux, scaling_only=False):
